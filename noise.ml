...
let random_int = Mersenne_twister.int
...
struct
  ...
  let octave_number = Config.octaves
  let size          = Config.psize
  ...
  struct
    ...
    (* gradients *)
    let gx = 
      Array.init size (fun i -> (random_float 2.0) -. 1.0)
	...
	then s
	  ...
	  in
	  let s = s +. (gen (x /. frequency)) *. amplitude
	  in
            calc_sum (succ i) s
      in
      let r = (calc_sum 0 0.)
  ...
  struct
	...
	(* Do the bi-cubic interpolation to get the final value *)
      ...
      and v1 = interpolate v10 v11 tx0
      in
  ...
  struct
	...
	(* Permutate values to get indices to use with the gradient look-up tables *)
      ...
      and q111 = p.((qz1 + p.((qy1 + p.(qx1 mod size)) mod size)) mod size)
      in
	(* Computing vectors from the eight points to the input point *)
      ...
      in
	(* Compute the dot-product between the vectors and the gradients *)
      let dot a (x2,y2,z2) = gx.(a) *. x2 +. gy.(a) *. y2 +. gz.(a) *. z2
      ...
      and v111 = dot q111 (tx1, ty1, tz1)
      in
	(* Do the tri-cubic interpolation to get the final value *)
      let v00 = interpolate v000 v001 tx0
      and v01 = interpolate v010 v011 tx0
      and v10 = interpolate v100 v101 tx0
      and v11 = interpolate v110 v111 tx0
      in
      let v0 = interpolate v00 v01 ty0
      and v1 = interpolate v10 v11 ty0
      in
	...
	then s
          ...
          and amplitude =  persistence ** (float i)
	  in
	  let s = s +. (gen (x /. frequency, y /. frequency, z /. frequency)) *. amplitude
	  in
            calc_sum (succ i) s
      in
      let r = (calc_sum 0 0.)
  ...
  struct
	...
	*)
	...
	then s
          ...
          and amplitude =  persistence ** (float i)
	  in
	  let v' = V.map (fun vi -> vi /. frequency) v
	  in
            calc_sum (succ i) (s +. (gen v') *. amplitude)
      in
      let r = (calc_sum 0 0.)
      in
	clamp r (-.1.) 1.
